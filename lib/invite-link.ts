...
export function invitationShareUrl(invite:{id:string;email:string}){return `${appBaseUrl()}/invite/${invite.id}?sig=${encodeURIComponent(invitationSignature(invite.id,invite.email))}`;}
