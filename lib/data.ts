...
export async function spaceSnapshot(spaceId: string, membership: typeof s.members.$inferSelect, selectedDate: string) {
  ...
  const shifts = parent ? shiftsAll : shiftsAll.filter(x=>x.memberId===membership.id);
  ...
  const accessibleProgram = programBase.filter(p => {
    ...
    return links.length===0 || links.some(id=>allowedChildIds.includes(id));
  });
  let program=accessibleProgram;
  let tasks = await db.select().from(s.tasks).where(and(eq(s.tasks.careSpaceId, spaceId), eq(s.tasks.taskDate, selectedDate))).orderBy(asc(s.tasks.time));
