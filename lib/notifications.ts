...
export async function notifySpaceUsers(args:{spaceId:string;excludeUserId?:string;kind:NotificationKind;type:string;title:string;body?:string;url?:string;childIds?:string[];memberIds?:string[]}){
  ...
  const memberIds=[...new Set((args.memberIds||[]).filter(Boolean))];if(memberIds.length)memberships=memberships.filter(member=>memberIds.includes(member.id));
  ...
  const safeUrl=args.type==="SHIFT_ENDED"||args.type==="ACTIVITY_DONE"?`/app?space=${args.spaceId}&section=today`:args.url||`/app?space=${args.spaceId}`;
  await db.insert(s.notifications).values(allowed.map(userId=>({userId,careSpaceId:args.spaceId,type:args.type,title:args.title,body:args.body||null})));await sendWebPush(allowed,{title:args.title,body:args.body,url:safeUrl,tag:`${args.type}-${args.spaceId}`});
}
