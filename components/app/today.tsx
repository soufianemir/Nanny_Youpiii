...
export function Today({spaceId,selectedDate,snapshot,memberName,viewMembership,parent,preview,readOnly=false,q,fullTeam,userFirstName,canActProgram,canActTasks,timezone}:{spaceId:string;selectedDate:string;snapshot:Snapshot;memberName:(member:Member)=>string;actualMembership:Member;viewMembership:Member;parent:boolean;preview:boolean;readOnly?:boolean;q:Query;fullTeam:Member[];userFirstName:string;canActProgram:boolean;canActTasks:boolean;timezone:string}){
  ...
  return <div className="v4-stack">
    ...
    <div className="v4-grid-2"><div className="v4-stack"><Card><SectionHeader title={parentView?"Aujourd’hui":"Ma journée"} eyebrow={rows.length?`${rows.filter(row=>row.status==="DONE").length}/${rows.length} terminé${rows.length>1?"s":""}`:undefined} action={<Link className="v4-text-action" href={q({section:"planning"})}>Planning</Link>}/>{rows.length?<div className="v4-timeline">{rows.map(row=><div key={`${row.kind}-${row.id}`} className={`v4-timeline-item ${row.status==="DONE"?"is-done":""}`}><time className="v4-timeline-time">{row.time||"—"}</time><span className="v4-timeline-rail"><span className="v4-timeline-icon"><Icon name={row.icon} size={18}/></span></span><span className="v4-timeline-copy"><strong>{row.title}</strong>{row.status==="DONE"?<small>Terminé</small>:row.subtitle&&<small>{row.subtitle}</small>}</span><span className="v4-timeline-action"><TimelineAction row={row} spaceId={spaceId} enabled={canInteract&&(row.kind==="program"?canActProgram:canActTasks)}/></span></div>)}</div>:<EmptyState title="Rien de prévu aujourd’hui" description="Profitez d’une journée tranquille."/>}</Card></div>
