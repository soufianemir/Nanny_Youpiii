...
function timeline(snapshot:Snapshot):TimelineRow[]{
  ...
  const tasks=snapshot.tasks.map(item=>({id:item.id,kind:"task" as const,time:item.time,title:item.title,subtitle:item.description||item.note||"",status:item.status,icon:activityIcon("tâche",item.title)}));
...
export function Today({spaceId,selectedDate,snapshot,memberName,viewMembership,parent,preview,readOnly=false,q,fullTeam,userFirstName,canActProgram,canActTasks,timezone}:{spaceId:string;selectedDate:string;snapshot:Snapshot;memberName:(member:Member)=>string;actualMembership:Member;viewMembership:Member;parent:boolean;preview:boolean;readOnly?:boolean;q:Query;fullTeam:Member[];userFirstName:string;canActProgram:boolean;canActTasks:boolean;timezone:string}){
  ...
  const canInteract=!preview&&!readOnly&&isToday;
  const parentView=parent&&!preview;
  const greeting=parentView?(userFirstName?`Bonjour ${userFirstName}`:"Bonjour"):`Bonjour ${memberName(viewMembership)}`;

  return <div className="v4-stack">
    <div className="v4-today-hero"><span className="v4-eyebrow">{dateLabel(selectedDate)}</span><h1>{greeting}</h1><p>{parentView?"Votre journée familiale, sans bruit inutile.":"Votre garde, vos prochaines actions et les informations utiles."}</p>{snapshot.shifts.length>0&&<div className="v4-shift-strip">{snapshot.shifts.map(shift=>{const member=shiftMember(shift.memberId);return <div className="v4-shift-row" key={shift.id}><Avatar name={memberName(member)} size="sm"/><span className="v4-row-copy"><strong>{memberName(member)}</strong><small>{shift.plannedStart} → {shift.plannedEnd}{shift.status==="ACTIVE"?" · garde en cours":""}</small></span>{shift.status==="ACTIVE"&&<StatusBadge tone="success">En cours</StatusBadge>}</div>})}</div>}</div>
    {next&&<Card className="v4-now-card" tone="brand"><span className="v4-eyebrow">Ensuite</span><div className="v4-section-header v4-next-heading"><div><h2>{next.time?`${next.time} · `:""}{next.title}</h2>{next.subtitle&&<small className="muted">{next.subtitle}</small>}</div><span className="v4-timeline-icon"><Icon name={next.icon}/></span></div></Card>}
    <div className="v4-grid-2"><div className="v4-stack"><Card><SectionHeader title={parentView?"Aujourd’hui":"Ma journée"} eyebrow={rows.length?`${rows.filter(row=>row.status==="DONE").length}/${rows.length} terminé${rows.length>1?"s":""}`:undefined} action={<Link className="v4-text-action" href={q({section:"planning"})}>Planning</Link>}/>{rows.length?<div className="v4-timeline">{rows.map(row=><div key={`${row.kind}-${row.id}`} className={`v4-timeline-item ${row.status==="DONE"?"is-done":""}`}><time className="v4-timeline-time">{row.time||"—"}</time><span className="v4-timeline-rail"><span className="v4-timeline-icon"><Icon name={row.icon} size={18}/></span></span>{row.kind==="program"?<Link href={q({section:"today",compose:`activity-edit-${row.id}`})} className="v4-timeline-copy v4-activity-row-link"><strong>{row.title}</strong>{row.status==="DONE"?<small>Terminé</small>:row.subtitle&&<small>{row.subtitle}</small>}</Link>:<span className="v4-timeline-copy"><strong>{row.title}</strong>{row.status==="DONE"?<small>Terminé</small>:row.subtitle&&<small>{row.subtitle}</small>}</span>}<span className="v4-timeline-action"><TimelineAction row={row} spaceId={spaceId} enabled={canInteract&&(row.kind==="program"?canActProgram:canActTasks)}/></span></div>)}</div>:<EmptyState title="Rien de prévu aujourd’hui" description="Utilisez le + pour ajouter une activité."/>}</Card></div>
