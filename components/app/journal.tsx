...
type JournalProps={spaceId:string;selectedDate:string;snapshot:Snapshot;team:Member[];memberName:(member:Member)=>string;canAdd:boolean;q:Query;compose?:string;kind?:string};
...
export function Journal({spaceId,selectedDate,snapshot,team,memberName,canAdd,q,compose,kind}:JournalProps){
  ...
  const quick=(journalKind:string)=>q({section:"journal",compose:"journal",kind:journalKind});
  return <div className="v4-stack"><PageTitle eyebrow="Journal" title={dateLabel(selectedDate)} description="Les moments utiles et les transmissions, dans l’ordre de la journée." action={canAdd?<Link className="btn brandbtn" href={quick("NOTE")}><Icon name="plus"/> Ajouter</Link>:undefined}/>
    ...
    {compose==="journal"&&<NoteSheet spaceId={spaceId} date={selectedDate} children={snapshot.children} q={q} kind={kind}/>} 
  </div>;
}
