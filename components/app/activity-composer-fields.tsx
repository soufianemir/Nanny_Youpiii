...
export function ActivityComposerFields({presets,children,initialChoice,initialChildIds,editing}:{presets:Preset[];children:Child[];initialChoice:string;initialChildIds:string[];editing:boolean}){
  ...
  function changeChoice(value:string){
    setChoice(value);
    const scoped=presetChildren(value,presets,children);
    if(scoped)setSelectedChildren(scoped);
    else if(children.length===1)setSelectedChildren([children[0].id]);
  }
  function toggleChild(id:string,checked:boolean){
    setSelectedChildren(current=>checked?[...new Set([...current,id])]:current.filter(value=>value!==id));
  }
  return <>
    <fieldset className="v4-activity-fieldset v52-activity-picker">
      ...
      <label className="v52-select-shell">
        <select name="activityType" value={choice} onChange={event=>changeChoice(event.target.value)} aria-label="Choisir une activité">
          ...
          {choice.startsWith("CURRENT::")&&<optgroup label="Activité actuelle"><option value={choice}>{choice.slice("CURRENT::".length)}</option></optgroup>}
