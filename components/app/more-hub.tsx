...
export function MoreHub({q,parent,canChildren,canTeam,canShopping,canCash,canRules,showSession=false}:{q:Query;parent:boolean;canChildren:boolean;canTeam:boolean;canShopping:boolean;canCash:boolean;canRules:boolean;showSession?:boolean}){
  ...
  return <div className="v4-stack"><PageTitle eyebrow="Nanny Youpiii" title="Plus" description={parent?"Les réglages et outils moins fréquents restent ici, à l’écart de votre quotidien.":"Seulement les outils auxquels vous avez accès."}/><Card><div className="v4-menu-list">{items.map(item=><Link className="v4-menu-row" key={item.area} href={q({section:"more",area:item.area})}><span className="v4-menu-row-icon"><Icon name={item.icon}/></span><span className="v4-menu-row-copy"><strong>{item.title}</strong><small>{item.description}</small></span><Icon name="chevronRight" size={18}/></Link>)}{items.length===0&&<div className="v4-menu-row"><span className="v4-menu-row-icon"><Icon name="check"/></span><span className="v4-menu-row-copy"><strong>Tout est prêt</strong><small>Vous n’avez aucun autre outil à gérer ici.</small></span></div>}</div></Card>{showSession&&<Card tone="soft"><SectionHeader title="Session"/><SignOutButton/></Card>}</div>;
}
