...
async function spaceToday(spaceId:string){
  ...
  return today(space.timezone);
}

export async function addInstructionAction(formData: FormData) {
  const spaceId = text(formData, "spaceId");
  const { session, membership } = await requireParentPermission(spaceId,"journal");
  const childIds=formData.getAll("childIds").map(String); const memberIds=formData.getAll("memberIds").map(String);
  await assertChildren(membership.id,childIds); await assertMembers(spaceId,memberIds); await assertMembersCanAccessChildren(spaceId,memberIds,childIds);
...
export async function addProgramItemAction(formData: FormData) {
  const spaceId = text(formData, "spaceId");
  const { session, membership } = await requireParentPermission(spaceId,"program");
  const childIds = formData.getAll("childIds").map(String);
  const memberIds = formData.getAll("memberIds").map(String);
  const date=text(formData,"date"), start=text(formData,"start")||null, end=text(formData,"end")||null;
  await assertChildren(membership.id, childIds); await assertMembers(spaceId, memberIds); await assertMembersCanAccessChildren(spaceId,memberIds,childIds);
  if(start&&memberIds.length) await assertWithinCareWindow(spaceId,memberIds,date,start,end);
  const [item] = await db.insert(s.programItems).values({
    careSpaceId: spaceId, programDate: date, type: text(formData, "type"), title: text(formData, "title"),
    description: text(formData, "description") || null, location: text(formData, "location") || null,
    plannedStart: start, plannedEnd: end, createdBy: session.user.id,
  }).returning();
...
export async function updateProgramStatusAction(formData: FormData) {
  ...
  let actualStart:Date|null=null, actualEnd:Date|null=null;
  if(status==="DONE"){
    const startText=text(formData,"actualStart")||item.plannedStart||"";
    const endText=text(formData,"actualEnd")||item.plannedEnd||startText;
    actualStart=localDateTime(item.programDate,startText)||new Date();
    actualEnd=localDateTime(item.programDate,endText)||actualStart;
    if(actualEnd.getTime()<actualStart.getTime()) throw new Error("INVALID_ACTUAL_TIME_RANGE");
  }
  ...
  await db.insert(s.activityLogs).values({ careSpaceId: spaceId, actorUserId: session.user.id, action: "PROGRAM_STATUS_CHANGED", entityType: "program_item", entityId: itemId, metadata: { status, actualStart, actualEnd } });
  revalidatePath("/app");
}

export async function addTaskAction(formData: FormData) {
  const spaceId = text(formData, "spaceId");
  const { session, membership } = await requireParentPermission(spaceId,"tasks");
  const assignees = formData.getAll("memberIds").map(String);
  const childIds = formData.getAll("childIds").map(String);
  const date=text(formData,"date")||null, time=text(formData,"time")||null;
  await assertChildren(membership.id, childIds); await assertMembers(spaceId, assignees); await assertMembersCanAccessChildren(spaceId,assignees,childIds);
  if(date&&time&&assignees.length) await assertWithinCareWindow(spaceId,assignees,date,time,time);
  const [task] = await db.insert(s.tasks).values({ careSpaceId: spaceId, taskDate: date, time, title: text(formData, "title"), description: text(formData, "description") || null, createdBy: session.user.id }).returning();
...
export async function addRoutineAction(formData: FormData) {
  const spaceId = text(formData, "spaceId");
  const { session } = await requireParentPermission(spaceId,"tasks");
...
export async function applyRoutineAction(formData: FormData) {
  ...
  if(childId) await assertMembersCanAccessChildren(spaceId,[memberId],[childId]);
