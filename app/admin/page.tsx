...
export default async function AdminPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const selectedSpaceMembers = selectedSpace ? await db.select().from(s.members).where(eq(s.members.careSpaceId, selectedSpace.id)) : [];

  const auditResult = await pool.query<AuditRow>(
    `select a.id, a.action, a.entity_type, a.entity_id, a.created_at,
            c.name as space_name, u.email as actor_email
  ...
  const [{ count: userCount }] = await db.execute<{ count: number }>(`select count(*)::int as count from neon_auth."user"`);
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      <section className={styles.hero}><div><span className={styles.eyebrow}>Back-office privé</span><h1>Administration</h1><p>Corrigez les accès et les données de support sans exposer ces outils aux parents ou aux intervenants. Toutes les mutations sont vérifiées côté serveur et journalisées.</p></div></section>

      <section className={styles.stats}>
        <div className={styles.stat}><strong>{Number(userCount || 0)}</strong><span>comptes Auth</span></div>
      ...
      <div className={styles.layout}>
        <section className={styles.card}>
          ...
          <div className={styles.userList}>{users.length ? users.map((user) => <Link key={user.id} href={`/admin?${new URLSearchParams({ ...(search ? { q: search } : {}), user: user.id, ...(selectedSpaceId ? { space: selectedSpaceId } : {}) })}`} className={`${styles.userRow} ${selectedUserId === user.id ? styles.userActive : ""}`}>
        ...
        <div className={styles.stack}>
          {selectedUser ? <section className={styles.card}>
            ...
            <p className={styles.notice}>La suspension globale bloque l’accès aux espaces sans supprimer l’historique. Elle est impossible sur votre propre compte ou si elle laisserait une famille sans Parent Admin.</p>
          </section> : <section className={styles.card}><div className={styles.empty}>Sélectionnez un utilisateur pour corriger ses rôles et permissions.</div></section>}
          ...
          {selectedUser && memberships.map(({ member, space }) => {
            ...
            return <section className={styles.membership} key={member.id}><div className={styles.membershipHead}><div><strong>{space.name}</strong><small>{roleLabel(member.role)} · {member.status === "ACTIVE" ? "actif" : "suspendu"}</small></div><span className={member.status === "ACTIVE" ? styles.badgeOk : styles.badgeWarn}>{member.status}</span></div>
              <form action={adminUpdateMembershipAction} className={styles.form}><input type="hidden" name="memberId" value={member.id}/>
                ...
                <div className={styles.field}><label>Libellé interne</label><input className={styles.input} name="label" defaultValue={member.label || ""} placeholder="Nounou journée, baby-sitter soir…"/></div>
                ...
                <div className={styles.actions}><button className={styles.buttonPrimary}>Enregistrer les modifications</button></div>
              </form>
              <div className={styles.dangerZone}><form action={adminRemoveMembershipAction}><input type="hidden" name="memberId" value={member.id}/><button className={styles.buttonDanger}>Retirer de cette famille</button></form><p className={styles.muted}>Si un historique financier existe, la suppression est refusée : utilisez alors Suspendu afin de préserver l’audit.</p></div>
            </section>;
          })}
        </div>
      </div>

      <div className={styles.layout} style={{ marginTop: 14 }}>
        <section className={styles.card}><div className={styles.sectionTitle}><h2>Familles</h2><small>{spaces.length}</small></div><div className={styles.spaceList}>{spaces.map((space) => {
          const children = allChildren.filter((child) => child.careSpaceId === space.id);
          return <Link className={styles.spaceRow} key={space.id} href={`/admin?${new URLSearchParams({ ...(search ? { q: search } : {}), ...(selectedUserId ? { user: selectedUserId } : {}), space: space.id })}`}><span><strong>{space.name}</strong><small>{children.length} enfant(s)</small></span><span className={styles.badge}>{selectedSpaceId === space.id ? "Ouvert" : "Gérer"}</span></Link>;
        })}</div></section>
        ...
        <div className={styles.stack}>{selectedSpace ? <>
          ...
          <section className={styles.card}><div className={styles.sectionTitle}><h2>Ajouter ou inviter</h2><small>{selectedSpace.name}</small></div><form className={styles.form}>
            ...
            <p className={styles.muted}>« Ajouter » fonctionne pour un compte déjà inscrit. « Inviter » envoie l’invitation Neon Auth et applique les droits choisis à l’acceptation.</p>
          </form></section>
